    ...
    <>
      <header className={colorChange ? 'navbar change' : 'navbar'}>
        <nav className={colorChange ? 'nav-menu change' : 'nav-menu'}>
          ...
          <div id={colorChange ? 'menu change' : 'menu'} className="flex items-center md:block sm:hidden">
            <Link onClick={handleAnchorClick} data-to-scrollspy-id="objective" className="px-3" to={ROUTES.OBJECTIVE}>關於比賽</Link>
